...
local anim = {}
anim.animation = class('goo animation')
anim.animation:include(Stateful)
anim.style = require 'anim.style'
anim.list = {}
anim.lastTime = 0

local style = require 'anim.style'
...
function finished:enteredState()
	self.startTime = nil
	self.table[self.key] = self.finish
...
function group:initialize(...)
	...
	for k,v in ipairs(arg) do
		...
		if instanceOf(anim.animation,v) or instanceOf(group, v) then
